...
async function createSDK(version) {
    ...
    sdkChannel.register("executeNewAction", async (payload, identity) => {
        console.log("Execute New Action: Requested by client: " + JSON.stringify(identity));
        console.log("Execute New Action: Payload sent in request: " + JSON.stringify(payload));
        return Math.floor(Math.random() * 1000);
    });
    sdkChannel.register("startStream", async (payload, identity) => {
        console.log("Start Stream Action: Requested by client: " + JSON.stringify(identity));
        console.log("Start Stream Action: Payload sent in request: " + JSON.stringify(payload));
        let response = await startStream(payload, identity, sdkChannel);
        return response;
    });
    sdkChannel.register("stopStream", async (payload, identity) => {
        console.log("Stop Stream Action: Requested by client: " + JSON.stringify(identity));
        console.log("Stop Stream Action: Payload sent in request: " + JSON.stringify(payload));
