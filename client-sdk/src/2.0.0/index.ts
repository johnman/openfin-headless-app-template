...
export async function executeAction(message:string) {
...
export async function executeNewAction(message:string) {
  if (isClientConnected()) {
...
export async function startStream(callback: (data:string)=>void, options: IStreamOptions) {
  if (isClientConnected()) {
    let streamId =
